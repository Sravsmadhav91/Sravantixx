...
import { deleteMigrationDocument, fetchMigrationDocumentBlob, listMigrationDocuments, type MigrationDocument } from "@/lib/migration-api.ts";
import MigrationUploadDocumentDialog from "./migration-upload-document-dialog.tsx";
import MigrationEditDocumentDialog from "./migration-edit-document-dialog.tsx";
...
export default function MigrationDocumentPanel({ linkedType, linkedId, linkedName }: Props) {
  const { isOwner } = useRole();
  const [docs, setDocs] = useState<MigrationDocument[] | undefined>(undefined);
  const [uploadOpen, setUploadOpen] = useState(false);
  const [editDoc, setEditDoc] = useState<MigrationDocument | null>(null);
  const [deleteDoc, setDeleteDoc] = useState<MigrationDocument | null>(null);
  const [previewDoc, setPreviewDoc] = useState<MigrationDocument | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);

  const reload = useCallback(() => {
    listMigrationDocuments({ linkedType, linkedId }).then(setDocs).catch(() => toast.error("Could not load documents"));
  }, [linkedType, linkedId]);
  ...
  useEffect(() => {
    ...
    fetchMigrationDocumentBlob(previewDoc._id).then((blob) => {
      if (!active) return;
  ...
  const handleDelete = async () => {
    if (!deleteDoc) return;
    try {
      await deleteMigrationDocument(deleteDoc._id);
  ...
  const handleDownload = async (doc: MigrationDocument) => {
    try {
      const blob = await fetchMigrationDocumentBlob(doc._id);
  ...
  return (
    <div className="space-y-3">
      ...
      {editDoc && <MigrationEditDocumentDialog document={editDoc} onClose={() => setEditDoc(null)} onUpdated={reload} />}
        ...
        <Dialog open onOpenChange={(o) => !o && setPreviewDoc(null)}>
          <DialogContent className="max-w-4xl max-h-[90vh] p-0 overflow-hidden">
            ...
            {!previewUrl ? (
              ...
              <img src={previewUrl} alt={previewDoc.label ?? previewDoc.fileName} className="w-full h-full object-contain max-h-[75vh]" />
            ) : (
              <iframe src={previewUrl} className="w-full" style={{ height: "75vh" }} title={previewDoc.label ?? previewDoc.fileName} />
            )}
          </DialogContent>
        </Dialog>
      )}
