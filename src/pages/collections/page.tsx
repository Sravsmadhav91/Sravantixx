...
function MigrationCollectionsPage() {
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-8 p-4 md:p-8"><div><h1 className="font-serif text-3xl font-semibold">Collections Dashboard</h1><p className="text-sm text-muted-foreground">Outstanding vs collected, overdue aging, and trends across all bookings.</p></div><div className="flex flex-wrap items-end gap-3 rounded-lg border border-border bg-card p-3"><select className="h-8 rounded-md border border-input bg-background px-3 text-xs" value={projectId} onChange={(e) => setProjectId(e.target.value)}><option value="all">All projects</option>{(projects ?? []).map((project) => <option key={project._id} value={project._id}>{project.name}</option>)}</select><Input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} className="h-8 w-36 text-xs" /><Input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="h-8 w-36 text-xs" /></div>{!kpis ? <Skeleton className="h-28 w-full" /> : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{[["Agreement value", formatCompactInr(kpis.totalAgreementValue)], ["Collected", formatCompactInr(kpis.totalCollected)], ["Outstanding", formatCompactInr(kpis.totalOutstanding)], ["Overdue", formatCompactInr(kpis.overdueAmount)], ["Due within 7 days", formatCompactInr(kpis.upcomingAmount)]].map(([label, value]) => <Card key={label}><CardContent className="space-y-2"><p className="text-xs uppercase text-muted-foreground">{label}</p><p className="text-2xl font-semibold">{value}</p><p className="text-xs text-muted-foreground">{label === "Collected" ? `${kpis.collectionRate}% of agreement value` : ""}</p></CardContent></Card>)}</div>}{dashboard && <div className="grid gap-6 lg:grid-cols-2"><Card><CardHeader><CardTitle>Overdue aging</CardTitle></CardHeader><CardContent className="space-y-2">{dashboard.aging.map((bucket) => <div key={bucket.label} className="flex justify-between rounded-md bg-muted/50 px-3 py-2 text-sm"><span>{bucket.label} ({bucket.count})</span><strong>{formatCompactInr(bucket.amount)}</strong></div>)}</CardContent></Card><Card><CardHeader><CardTitle>Collections trend</CardTitle></CardHeader><CardContent className="space-y-2">{dashboard.trend.map((row) => <div key={row.sortKey} className="flex justify-between border-b py-2 text-sm"><span>{row.month}</span><strong>{formatCompactInr(row.collectedAmount)}</strong></div>)}</CardContent></Card></div>}{dashboard && <Card><CardHeader><CardTitle>Outstanding by project</CardTitle></CardHeader><CardContent className="space-y-3">{dashboard.byProject.map((project) => <div key={project.projectId} className="flex justify-between border-b py-2 text-sm"><span>{project.name}</span><div><strong>{formatCompactInr(project.collectedAmount)}</strong><span className="ml-3 text-destructive">{formatCompactInr(project.outstanding)}</span></div></div>)}</CardContent></Card>}<div><h2 className="font-serif text-2xl font-semibold">Bookings</h2><p className="text-sm text-muted-foreground">Payment schedules and receipts for every booking.</p></div>{alerts?.overdue.length ? <OverduePanel items={alerts.overdue} readOnly /> : null}{alerts?.upcoming.length ? <UpcomingDuePanel items={alerts.upcoming} readOnly /> : null}<div className="flex gap-2">{(["all", "active", "cancelled"] as const).map((s) => <button key={s} onClick={() => setFilter(s)} className="rounded-md bg-secondary px-3 py-1.5 text-sm capitalize">{s}</button>)}</div>{bookings === undefined ? <Skeleton className="h-24 w-full" /> : <div className="space-y-3">{bookings.map((booking) => <Link key={booking._id} to={`/collections/${booking._id}`} className="block rounded-lg border bg-card p-4"><span className="font-medium">{booking.unit.projectName} · {booking.unit.number}</span><p className="text-sm text-muted-foreground">{booking.buyer.name} · {formatCompactInr(booking.agreementValue)}</p></Link>)}</div>}</div>;
}
        ...
        <>
          <div className="space-y-3">
            {filtered.map((booking) => {
              const hasOverdue = overdueBookingIds.has(booking._id as string);
              return (
                <Link
                  key={booking._id}
