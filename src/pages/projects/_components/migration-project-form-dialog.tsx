...
type ProjectFormProject = { _id: string; name: string; code: string; city: string; address?: string; type?: string; status?: string; reraNumber?: string; possessionDate?: string; constructionBudget?: number; notes?: string };
...
export default function MigrationProjectFormDialog({ open, onOpenChange, project }: { open: boolean; onOpenChange: (open: boolean) => void; project?: ProjectFormProject }) {
  ...
  const submit = async () => {
    ...
    try {
      if (project) {
        await updateMigrationProject(project._id, { name, code, city, type, status, reraNumber, possessionDate, constructionBudget: constructionBudget ? Number(constructionBudget) : undefined, address, notes });
        toast.success("Project updated");
      } else {
        await createMigrationProject({ name, code, city, type, status, reraNumber, possessionDate, constructionBudget: constructionBudget ? Number(constructionBudget) : undefined, address, notes });
